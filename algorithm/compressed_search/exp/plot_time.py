...
methods = ['LZ-VLH(our)', 'LZ77','LZ4(speed)', 'LZ4(ratio)', 'GZIP', 'Snappy', 'LZMA']
...
df = pd.read_csv('D:\\2025\\DQ\\research\\compressed_search\\compressed_search\\exp\\original_data\\encode_time_all_datasets.csv', header=None)  # 没有标题行
...
for i in range (len(datasets)):
    for j in range (len(methods_new)):
        timeRatio_new[i][j] = timeRatio[i][j]
        valueRatio_new[i][j] = valueRatio[i][j]
        timePredictRatio_new[i][j] = timePredictRatio[i][j]
        if j==1:
            timeRatio_new[i][j] = timeRatio_new[i][j] * 100
            valueRatio_new[i][j] = valueRatio_new[i][j] * 100
            timePredictRatio_new[i][j] = timePredictRatio_new[i][j] * 100

for i in range(len(methods_new)):
    ax.bar(x + i * bar_width, valueRatio_new[:, i]+timePredictRatio_new[:, i], width=bar_width, label=methods_new[i], color=colors[i])
...
plt.tight_layout()
plt.savefig('D:\\2025\\DQ\\research\\compressed_search\\compressed_search\\exp\\compression_time_alldatasets_new.png')