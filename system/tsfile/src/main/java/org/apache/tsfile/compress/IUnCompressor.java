...
public interface IUnCompressor {
  ...
  public class LzVlhDecompressor implements IUnCompressor {

    private static final Logger logger = LoggerFactory.getLogger(LzVlhDecompressor.class);
    private static final String UNCOMPRESS_INPUT_ERROR =
        "tsfile-compression LzVlhDecompressor: error occurs when uncompressing input byte";
    ...
    public LzVlhDecompressor() {}

    @Override
    public int getUncompressedLength(byte[] array, int offset, int length) {
      throw new UnsupportedOperationException("Unsupported getUncompressedLength");
    }
    ...
    /**
     * Don't use unless you don't know uncompressed length. Will allocate MAX_COMPRESS_RATIO *
     * compressed size.
     */
    @Override
    public byte[] uncompress(byte[] bytes) throws IOException {
    ...
    public int uncompress(byte[] byteArray, int offset, int length, byte[] output, int outOffset)
        throws IOException {
      try {
        decompressor.decompress(byteArray, offset,output,outOffset, output.length);
    ...
    @Override
    public CompressionType getCodecName() {
      return CompressionType.LZVLH; // 确保你有这个枚举类型或改为合适名称
    }
  }
